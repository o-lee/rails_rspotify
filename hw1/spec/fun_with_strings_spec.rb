...
describe 'anagram grouping' do
  ...
  it 'for "peek clams MoonStarer keep calms Astronomer"' do
    @anagrams = 'peek clams MoonStarer keep calms Astronomer'.anagram_groups
    @anagrams.each { |group| group.sort! }
    [%w(keep peek), %w(calms clams), %w(Astronomer MoonStarer)].each do |group|
      expect(@anagrams).to include(group)
    end
  end
